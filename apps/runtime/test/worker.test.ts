...
import { processResetStorage } from "../src/application/reset";
...
describe("Worker foundation", () => {
  ...
  it("serves Cloudflare-only owner sign-in at the root", async () => {
    const response = await exports.default.fetch("https://later-gator.test/");
    expect(response.status).toBe(200);
    const html = await response.text();
    expect(html).toContain("Continue with Cloudflare");
    expect(html).toContain("does not use a separate password or recovery phrase");
    expect(html).not.toContain("Later Gator password");
    expect(html).toContain('rel="icon"');
    expect(html).toContain('href="/img/icon-transparent.png"');
    expect(html).toContain('rel="apple-touch-icon"');
    expect(html).not.toContain('type="password"');
  });

  it("uses ordinary sessions without password-derived encryption material", async () => {
    const client = await login();
    const columns = await env.DB.prepare("PRAGMA table_info(sessions)").all<{ name: string }>();
    expect(columns.results.map((column) => column.name)).not.toContain("encrypted_data_key");
    expect(columns.results.map((column) => column.name)).not.toContain("data_key_nonce");
    const response = await exports.default.fetch("https://later-gator.test/", {
  ...
  it("starts one installation-bound owner login without storing raw state or nonce", async () => {
    const response = await exports.default.fetch("https://later-gator.test/auth/login", {
      redirect: "manual",
    });
    expect(response.status).toBe(302);
    const destination = new URL(response.headers.get("location") ?? "");
    expect(destination.origin).toBe("https://control-plane.test");
    expect(destination.pathname).toBe("/runtime/login");
    expect(destination.searchParams.get("installation_id")).toBe("test-installation-0001");
    expect(destination.searchParams.get("callback")).toBe(
      "https://later-gator.test/auth/callback",
    );
    const state = destination.searchParams.get("state");
    const nonce = destination.searchParams.get("nonce");
    expect(state).toMatch(/^[A-Za-z0-9_-]{40,}$/u);
    expect(nonce).toMatch(/^[A-Za-z0-9_-]{40,}$/u);
    const row = await env.DB
      .prepare("SELECT state_hash, nonce_hash FROM owner_login_requests")
      .first<{ state_hash: string; nonce_hash: string }>();
  ...
  it("keeps thumbnail storage owner-only and requires CSRF for every change", async () => {
    const unauthenticated = await exports.default.fetch(
      "https://later-gator.test/api/storage/thumbnails",
    );
    ...
    expect(await status.json()).toMatchObject({
      storage: { mode: "kv", status: "ready", migrationId: null },
    });

    const withoutCsrf = await exports.default.fetch(
      "https://later-gator.test/api/storage/thumbnails/disable",
      { method: "POST", headers: { cookie: client.cookie } },
    );
    expect(withoutCsrf.status).toBe(403);

    const disabled = await exports.default.fetch(
      "https://later-gator.test/api/storage/thumbnails/disable",
      { method: "POST", headers: mutationHeaders(client) },
    );
    expect(disabled.status).toBe(200);
    expect(await disabled.json()).toMatchObject({ storage: { mode: "disabled" } });

    const enabled = await exports.default.fetch(
      "https://later-gator.test/api/storage/thumbnails/enable-kv",
      { method: "POST", headers: mutationHeaders(client) },
    );
    expect(enabled.status).toBe(200);
    expect(await enabled.json()).toMatchObject({ storage: { mode: "kv" } });

    const migration = await exports.default.fetch(
      "https://later-gator.test/api/storage/thumbnails/migrate-r2",
      { method: "POST", headers: mutationHeaders(client) },
    );
    expect(migration.status).toBe(409);
    expect(await migration.json()).toMatchObject({
      error: { code: "thumbnail_storage_unavailable" },
    });
  });
  ...
  it("serves standard library dialogs and expires unauthenticated API cookies", async () => {
    ...
    const chromeGuide = await exports.default.fetch("https://later-gator.test/extension/chrome", {
      headers: { cookie: client.cookie },
    });
    expect(chromeGuide.status).toBe(200);
    expect(await chromeGuide.text()).toContain("Install the Chrome extension");
    const removedFirefoxGuide = await exports.default.fetch(
      "https://later-gator.test/extension/firefox",
      { headers: { cookie: client.cookie } },
    );
    expect(removedFirefoxGuide.status).toBe(404);
  ...
  it("supports permanent-folder manual saves, export, trash, and permanent deletion", async () => {
    ...
    const missingThumbnailId = await exports.default.fetch(
      `https://later-gator.test/api/thumbnails/${created.bookmark.id}`,
      { headers: { cookie: client.cookie } },
    );
    expect(missingThumbnailId.status).toBe(404);
