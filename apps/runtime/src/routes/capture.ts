...
const extensionCaptureSchema = z.strictObject({
  ...
  /** Set only after the extension has shown the owner the duplicate warning. */
  acceptExistingPostLinks: z.boolean().optional(),
});
...
/** Matches the four individually selectable links shown by the extension. */
const MAX_POST_LINKS = 4;
...
/** Finds selected post destinations that already exist without changing the library. */
async function existingPostLinks(
  db: D1Database,
  destinations: string[],
): Promise<ExistingPostLink[]> {
  const existing: ExistingPostLink[] = [];
  for (const destination of destinations) {
    const normalized = normalizeBookmarkUrl(destination);
    const bookmark = await db
      .prepare(
        `SELECT url, title, hostname
           FROM bookmarks
          WHERE normalized_url = ? AND deleted_at IS NULL
          LIMIT 1`,
      )
      .bind(normalized.normalizedUrl)
      .first<ExistingPostLink>();
    if (bookmark !== null) existing.push(bookmark);
  }
  return existing;
}
...
export async function captureBookmark(
  ...
  try {
    const postDestinations =
      kind === "extension" && extensionData?.success === true
        ? await resolvePostLinks(extensionData.data.postLinks ?? [])
        : [];
    if (
      extensionData?.success === true &&
      extensionData.data.acceptExistingPostLinks !== true &&
      postDestinations.length > 0
    ) {
      const duplicates = await existingPostLinks(env.DB, postDestinations);
          ...
          json(
            {
              ok: false,
              error: {
                code: "x_destination_already_saved",
                message: "One or more selected links are already in Later Gator.",
              },
              duplicates,
            },
            { status: 409 },
    ...
    if (kind === "extension" && extensionData?.success === true) {
      if (postDestinations.length > 0) {
        await linkXPostDestinations(env, source.bookmark.id, postDestinations);
      }
    }
