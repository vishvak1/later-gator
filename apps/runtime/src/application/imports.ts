...
import type { ImportSession } from "../domain/library-state";
...
export interface StartedImport {
  status: ImportSession;
  completion: Promise<void>;
}

/** Parses csv for CSV import. */
...
/** Divides parsed CSV bookmarks into bounded D1 batch sizes. */
...
/** Parses tags for CSV import. */
function parseTags(value: string): string[] {
  return value.trim() === "" ? [] : normalizeTagNames(value.split(/[,;]/u), 50);
}

/** Marks import stopped for CSV import. */
...
/** Inserts one import chunk idempotently and returns created and skipped counts. */
async function insertBookmarks(
  ...
  try {
    ...
    await db
      ...
      .run();
    await Promise.all([
      env.BACKGROUND_QUEUE.send({ type: "dispatch_pending" }),
...
/** Starts raindrop csv import for CSV import. */
...
/** Retrieves import status for CSV import. */
export async function getImportStatus(db: D1Database, importId: string): Promise<ImportSession | null> {
  return db
    .prepare(
