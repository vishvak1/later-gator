import { z } from "zod";
import { WORKERS_AI_LIMIT_CODE } from "../domain/provider-status";
...
/**
 * Workers AI models may answer with either `{ response }` or
 * `{ choices: [{ message: { content } }] }`, and some return both. Reading only
 * the direct response shape
...
export function normalizeWorkersAiResult(raw: unknown): unknown {
  const direct = z.looseObject({ response: z.unknown() }).safeParse(raw);
  if (direct.success && direct.data.response !== undefined && direct.data.response !== null) {
 ...
 *
 * Capacity and rate-limit phrasings are matched here too. They are the same
 * situation from the owner's side — the account cannot run the model right now
 * and waiting fixes it — and classifying them apart is what produced two
 * different messages for one condition, depending on which phrasing came back.
 */
const ALLOCATION_EXHAUSTED =
  /\b(?:3036|4006|3040|429)\b|daily free allocation|daily.*(?:limit|allocation)|allocation.*exhaust|neurons.*upgrade|capacity temporarily exceeded|rate.?limit|too many requests/iu;
...
/** Reads and parses a provider JSON response within the configured byte limit. */
...
/** Maps an HTTP status to the safe provider error category used by the app. */
...
/** Sends a bounded external provider request and returns validated JSON input. */
...
/** Calls the active provider and returns its normalized structured response. */
  ...
  if (provider === "workers-ai") {
    try {
      ...
      if (ALLOCATION_EXHAUSTED.test(message)) {
        throw new OrganizationProviderError("allocation", WORKERS_AI_LIMIT_CODE);
      }
...
/** Verifies that a provider and model can return a minimal valid response. */
