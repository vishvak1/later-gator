...
/** Parses form password for Worker request and queue routing. */
...
/** Authenticates a password, records failures, and creates a dashboard session. */
async function login(request: Request, env: Env): Promise<Response> {
  ...
  const session = await createSession(env.DB, authentication.rawDataKey);
  const headers = new Headers();
  headers.append("set-cookie", session.cookie);
  headers.append(
    "set-cookie",
    `lg_csrf=${session.csrfToken}; Path=/; Secure; SameSite=Lax; Max-Age=${(
      14 *
      24 *
      60 *
      60
...
async function handleFetch(
  ...
  if (request.method === "GET" && url.pathname === "/health") {
    return json({ ok: true, service: "later-gator", version: "1.0.0" });
  }
  ...
  if (session === null) {
    if (url.pathname.startsWith("/api/")) {
      ...
      response.headers.append(
        "set-cookie",
        "lg_csrf=; Path=/; Secure; SameSite=Lax; Max-Age=0",
      );
      return response;
    }
  ...
  if (request.method === "POST" && url.pathname === "/auth/logout") {
    ...
    headers.append("set-cookie", "lg_csrf=; Path=/; Secure; SameSite=Lax; Max-Age=0");
    return new Response(null, { status: 303, headers });
  }
