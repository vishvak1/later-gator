...
/** Renders the signed-out password form with CSRF-safe client behavior. */
  ...
  return page(
    ...
    `<main class="auth-shell">
      <section class="auth-card space-y-5">
        ...
        <form id="loginForm" class="stack" method="post" action="/auth/login" novalidate>
          <label for="loginPassword">Later Gator password</label>
          <div class="password-field">
            <input id="loginPassword" name="password" type="password" autocomplete="current-password"
              required autofocus aria-describedby="loginValidation">
            <button id="loginReveal" class="reveal-button" type="button" aria-pressed="false"
              aria-controls="loginPassword" title="Show password">Show</button>
          </div>
          <p id="loginValidation" class="status error" role="alert" hidden></p>
      ...
      <section class="auth-card oauth-consent-card space-y-5">
        ...
        <form class="stack" method="post" action="/authorize">
          ...
          ${options.signedIn
            ...
            : `<label for="oauthPassword">Later Gator password<input id="oauthPassword" name="password" type="password" autocomplete="current-password" required autofocus></label>`}
