...
export const HOW_TO_PANELS: readonly { id: string; kicker: string; title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: `<ol class="setup-steps">
        ...
        <li><strong>Load it.</strong><span>Choose <strong>Load Temporary Add-on</strong> and select <code>extension/firefox/manifest.json</code>.</span></li>
        <li><strong>Connect once.</strong><span>Generate a connection code in Settings, then paste it into the toolbar popup.</span></li>
      </ol>
      <p class="muted">Firefox drops temporary add-ons when it restarts, so this has to be repeated after a browser restart.</p>`,
  },
  {
    id: "ios",
  ...
  },
  {
    ...
    body: `<p class="muted">The MCP URL grants read-only access to search and inspect your library. The secret is inside the URL.</p>
      <ol class="setup-steps">
        <li><strong>Generate the URL.</strong><span>In Settings, choose Generate or rotate MCP URL and copy it. It is shown only once.</span></li>
        <li><strong>Add a remote MCP server.</strong><span>In your client's connector settings, add a custom or remote MCP server.</span></li>
        <li><strong>Paste the whole URL.</strong><span>Do not add separate authentication; the URL carries it.</span></li>
        <li><strong>Rotate if exposed.</strong><span>Generating a new URL immediately invalidates the old one.</span></li>
      </ol>`,
  },
  {
    id: "models",
