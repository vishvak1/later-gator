...
export type ThumbnailIngestOutcome =
  | "candidate_rejected"
  | "storage_disabled"
  | "storage_paused"
  | "stored";
...
): Promise<ThumbnailIngestOutcome> {
  if (isPlaceholderThumbnail(candidateUrl)) return "candidate_rejected";
  try {
    const storageState = await readThumbnailStorageState(env.DB);
    if (storageState.mode === "disabled") return "storage_disabled";
    if (storageState.status === "paused") return "storage_paused";
    const store = thumbnailStore(storageState.mode, runtimeThumbnailBindings(env));
    ...
    if (!hasSupportedImageSignature(inputBytes)) return "candidate_rejected";
    ...
    if (outputBytes === null) return "candidate_rejected";
    if (outputBytes.byteLength === 0) return "candidate_rejected";

    const thumbnailId = crypto.randomUUID();
    const objectKey = `thumbnails/${bookmarkId}/${thumbnailId}.webp`;
    const old = await env.DB
      .prepare("SELECT object_key, storage_backend FROM thumbnails WHERE bookmark_id = ?")
      .bind(bookmarkId)
      .first<{ object_key: string; storage_backend: "kv" | "r2" }>();
    const now = new Date().toISOString();
    const sourceHash = await sha256Base64(candidateUrl);
    const etag = `"sha256-${await sha256Base64(outputBytes)}"`;
    await store.put(objectKey, outputBytes);
    try {
      await env.DB.batch([
        env.DB
          .prepare(
    ...
    if (
      old !== null &&
      old.object_key !== objectKey &&
      old.storage_backend === store.backend
    ) {
      await store.delete(old.object_key);
    }
    return "stored";
  } catch (error: unknown) {
    if (error instanceof ThumbnailStorageFailure) {
      if (error.code === "thumbnail_storage_disabled") return "storage_disabled";
      await pauseThumbnailStorage(env.DB, error);
      return "storage_paused";
    }
    return "candidate_rejected";
  }
}
