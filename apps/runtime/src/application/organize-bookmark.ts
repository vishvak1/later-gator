...
import { stageXDestinationReview } from "./x-destination-review";
import {
  logRetrievalDiagnostic,
  retrievalDiagnostic,
...
export async function organizeBookmarkJob(
  ...
  const diagnostic: RetrievalDiagnostic = retrievalDiagnostic(
    pageContext,
  ...
  await env.DB
    .prepare(
      `INSERT INTO organization_diagnostics (job_id, summary_json, updated_at)
       VALUES (?, ?, ?)
       ON CONFLICT(job_id) DO UPDATE SET
         summary_json = excluded.summary_json,
         updated_at = excluded.updated_at`,
    )
    .bind(context.job_id, JSON.stringify(diagnostic), new Date().toISOString())
    .run();
  logRetrievalDiagnostic(
    context.job_id,
    context.bookmark_id,
    diagnostic,
    hasEvidence() ? null : "content_unavailable",
  );
  if (!hasEvidence()) {
  ...
  try {
    ...
    if (applied) {
      if (pageContext?.xPost !== null && pageContext?.xPost !== undefined) {
        const staged =
          context.source_type === "ios" && pageContext.xPost.externalUrls.length > 0
            ? await stageXDestinationReview(
                env.DB,
                context.job_id,
                context.bookmark_id,
                pageContext.xPost.externalUrls,
              )
            : false;
        if (staged) {
          logRetrievalDiagnostic(
