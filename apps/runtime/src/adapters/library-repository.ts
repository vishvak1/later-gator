...
import type { AutomationProgress, ImportSession, ProviderState } from "../domain/library-state";
import { z } from "zod";
import { runtimeThumbnailBindings, thumbnailStore } from "./thumbnail-store";
import { retrievalDiagnosticSchema } from "../observability/retrieval";
...
/** Retrieves app state for the D1 library repository. */
...
/** Retrieves provider for the D1 library repository. */
...
/** Retrieves bookmark for the D1 library repository. */
  ...
  return db
    .prepare(
      `SELECT b.*, f.name AS folder_name,
              ...
              (SELECT j.last_safe_error_code FROM background_jobs j
                WHERE j.bookmark_id = b.id AND j.state IN ('review', 'failed')
                ORDER BY j.updated_at DESC LIMIT 1) AS review_reason,
              (SELECT j.id FROM background_jobs j
                WHERE j.bookmark_id = b.id AND j.state IN ('review', 'failed')
                ORDER BY j.updated_at DESC LIMIT 1) AS diagnostic_id,
              (SELECT d.summary_json
                 FROM background_jobs j
                 JOIN organization_diagnostics d ON d.job_id = j.id
                WHERE j.bookmark_id = b.id AND j.state IN ('review', 'failed')
                ORDER BY j.updated_at DESC LIMIT 1) AS diagnostic_summary
         FROM bookmarks b
...
/** Commits the owner profile and provider choices that complete initial setup. */
...
/** Updates personal instructions for the D1 library repository. */
...
/** Creates bookmark for the D1 library repository. */
...
/** Encodes bookmark cursor for the D1 library repository. */
...
/** Decodes bookmark cursor for the D1 library repository. */
...
/** Lists bookmark page for the D1 library repository. */
...
/** Lists bookmarks for the D1 library repository. */
...
/** Updates bookmark for the D1 library repository. */
...
/** Sets bookmark deleted for the D1 library repository. */
...
/** Retrieves bootstrap state for the D1 library repository. */
...
/** Creates tag for the D1 library repository. */
...
/** Retires tag for the D1 library repository. */
...
/** Sets owner pause for the D1 library repository. */
...
/** Creates a symmetric bookmark relationship after validating both endpoints. */
...
/** Removes the symmetric relationship between two bookmarks. */
...
/** Restores tag for the D1 library repository. */
...
/** Deletes a trashed bookmark and all database-owned dependent records. */
export async function permanentlyDeleteBookmark(
  ...
  const bookmark = await env.DB
    .prepare(
      `SELECT b.id, t.object_key, t.storage_backend
         FROM bookmarks b
         LEFT JOIN thumbnails t ON t.bookmark_id = b.id
        WHERE b.id = ? AND b.deleted_at IS NOT NULL`,
    )
    .bind(bookmarkId)
    .first<{ id: string; object_key: string | null; storage_backend: "kv" | "r2" | null }>();
  if (bookmark === null) return false;
  if (bookmark.object_key !== null && bookmark.storage_backend !== null) {
    await thumbnailStore(bookmark.storage_backend, runtimeThumbnailBindings(env))
      .delete(bookmark.object_key);
  }
  await env.DB.prepare("DELETE FROM bookmarks WHERE id = ? AND deleted_at IS NOT NULL").bind(bookmarkId).run();
  return true;
}

/** Retrieves bookmark details for the D1 library repository. */
export async function getBookmarkDetails(
  ...
  if (bookmark.diagnostic_summary !== null && bookmark.diagnostic_summary !== undefined) {
    try {
      const parsed = retrievalDiagnosticSchema.safeParse(
        JSON.parse(bookmark.diagnostic_summary) as unknown,
      );
      diagnosticSummary = parsed.success ? parsed.data : null;
    } catch {
      diagnosticSummary = null;
    }
  }
  const publicBookmark = { ...bookmark };
  delete publicBookmark.diagnostic_summary;
