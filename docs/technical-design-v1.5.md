# Later Gator — Technical Design Document v1.5

**Status:** Ready for implementation after review  
**Product requirements:** Later Gator PRD v5.5  
**Supersedes:** Technical Design v1.4  
...
Bootstrap deployment secret:

| Secret | Purpose |
|---|---|
| `INSTALLATION_SECRET` | User-chosen password of at least 10 characters for the setup and administration page and the root material used to encrypt credentials entered there |

Automatically managed secret:

| Secret | Purpose |
|---|---|
| Encrypted `mcpPath` credential | Cryptographically random 64-character segment generated on the first authenticated setup visit and replaced by the one-click connection-address rotation action |
...
- `.dev.vars.example` declares only `INSTALLATION_SECRET` for deployment and local development. Raindrop and external-provider credentials are entered later in `/setup`; the MCP secret is generated by the application.
...
5. A plain-language explanation of the single 10-character-minimum setup password shown by the deploy form and the automatically generated MCP connection address.
...
- Generate a 64-character cryptographically random path secret with Web Crypto on the first authenticated setup visit, encrypt it with the credential store, and display only the complete copyable connection URL in the authenticated page. Hash both the supplied and configured values to equal-length digests, then compare those digests in constant time.
...
- Rotate the secret through the authenticated **Generate a new connection address** action, then update the MCP client with the newly displayed address.
...
| MCP | Display and copy the automatically generated MCP URL, provide ChatGPT and Claude setup snippets, generate a new connection address without exposing secret management, and run a safe `get_context()` connection test |
...
- Store the bootstrap installation password using the Deploy to Cloudflare secret prompt or the deployed Worker's Cloudflare dashboard secret settings.
- Generate the MCP path secret with Web Crypto after authenticated setup access, encrypt it in KV, and regenerate it only through the authenticated rotation action.
...
- Generate MCP and session secrets with cryptographically secure randomness. The user chooses the installation password with a 10-character minimum.
...
- This design v1.5 and PRD v5.5 are accepted together.
