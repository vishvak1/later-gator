...
import { normalizeWorkersAiResult, OrganizationProviderError, runOrganizationProvider } from "../src/adapters/organization-providers";
import { resetApplication } from "../src/application/reset";
import { allocationRetryAfter, organizeBookmarkJob } from "../src/application/organize-bookmark";
import {
  isWorkersAiLimit,
  providerStatusMessage,
  WORKERS_AI_LIMIT_CODE,
} from "../src/domain/provider-status";
import {
  ...
  listingEntries,
  type PageContext,
} from "../src/application/page-content";
import { repairThumbnailBacklog } from "../src/application/thumbnail-jobs";
import {
...
describe("sequential organization", () => {
  ...
  it("keeps AI/background and thumbnail queue messages distinct", () => {
    ...
    const thumbnailMessage = { type: "thumbnail", jobId };
  ...
  it("refreshes and retries a job when the bookmark revision is stale", async () => {
    ...
    /*
     * Distinguished from a plain retry so the queue can reschedule it in
     * seconds rather than the provider back-off of five minutes: nothing is
     * unavailable here, the revision simply moved while the model was thinking.
     * Sharing the slow path is what left bookmarks sitting in Unsorted long
     * after they had been organized.
     */
    expect(outcome).toBe("retry_soon");
...
describe("Workers AI response envelopes", () => {
  it("reads both the direct and chat-completions shapes", () => {
    // Some models answer with { response }.
    ...
    // Models may send both; the direct field stays authoritative.
    expect(normalizeWorkersAiResult({
...
describe("reset and the vector index", () => {
  it("forgets the vectors of every bookmark it deletes", async () => {
    ...
    await resetApplication(resetEnv, "session-hash", "https://later-gator.test");
...
describe("Workers AI allocation errors", () => {
  it("recognises the exhausted daily allowance from its real wording", async () => {
    ...
    )).toBe(WORKERS_AI_LIMIT_CODE);
    // The historical wording must keep working.
    expect(await classify("3036: daily limit exceeded")).toBe(WORKERS_AI_LIMIT_CODE);
    // Capacity and rate-limit phrasings are the same situation for the owner —
    // the account cannot run the model now and waiting fixes it. Classifying
    // them apart is what produced two different messages for one condition.
    expect(await classify("3040: capacity temporarily exceeded")).toBe(WORKERS_AI_LIMIT_CODE);
    expect(await classify("429 Too Many Requests")).toBe(WORKERS_AI_LIMIT_CODE);
    // A genuine model fault is still temporary, not a limit.
    expect(await classify("no such model: @cf/does/not-exist")).toBe("workers_ai_temporary");
  });

  it("gives one message for the limit, and schedules its own recovery", () => {
    // One condition, one sentence, wherever it surfaces.
    expect(providerStatusMessage(WORKERS_AI_LIMIT_CODE)).toContain("Workers AI limit has been reached");
    expect(isWorkersAiLimit(WORKERS_AI_LIMIT_CODE)).toBe(true);
    expect(isWorkersAiLimit("workers_ai_temporary")).toBe(false);

    // A spent allowance comes back at UTC midnight, so the pause lifts itself.
    const noon = new Date("2026-08-09T12:34:56.000Z");
    expect(
      allocationRetryAfter(new OrganizationProviderError("allocation", WORKERS_AI_LIMIT_CODE), noon),
    ).toBe("2026-08-10T00:00:00.000Z");
    // A wrong model ID never heals on a clock, so it stays waiting for the owner.
    expect(
      allocationRetryAfter(new OrganizationProviderError("systemic", "provider_http_404"), noon),
    ).toBeNull();
  });
});
